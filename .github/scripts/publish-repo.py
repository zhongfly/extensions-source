...
# Drop the previous git-hosted apk/jar layout. APK/JAR now live in GitHub Releases;
# icons are served from source files on the main branch.
for name in ("apk", "jar", "icon", "index.min.json"):
    path = REPO_DIR / name
    if path.is_dir():
        shutil.rmtree(path)
    elif path.is_file():
...
ICON_BASE_URL = "https://cdn.jsdelivr.net/gh/zhongfly/extensions-source@main"
...
# sibling in the same build dir.
new_extensions: list[tuple[index_pb2.Extension, Path, Path, bool, bool, dict]] = []
...
for info_file in ARTIFACTS_DIR.glob("**/keiyoushi-source-info.json"):
    ...
    new_extensions.append((ext, apk, jar, apk_changed, jar_changed, assets))

new_extensions.sort(key=lambda item: item[0].packageName)

updated_release_assets = {
    ext.packageName: assets
    for ext, _, _, _, _, assets in new_extensions
...
changed_index = 0
for ext, apk, jar, apk_changed, jar_changed, _ in new_extensions:
...
# Full rebuild: published index is exactly the extensions built in this run.
final_extensions = [
    ext
    for ext, _, _, _, _, _ in new_extensions
    if not any(ext.packageName.endswith(f".{module}") for module in to_delete)
]
...
index = index_pb2.Index(
    name="zhongfly",
    badgeLabel="ZF",
    signingKey="3bcded04aac982df40188edc5f28091523f3972a87d193c7111f9a1912b180a4",
    contact=index_pb2.Contact(
        website="https://github.com/zhongfly/extensions-source"
    ),
...
def create_release(tag: str):
    ...
    run_gh(
        ...
        "--notes",
        f"Automated update from zhongfly/extensions-source@{current_sha}",
    )
...
for i in range(0, total_changed_extensions, ext_per_release):
    ...
    files_to_upload = [
        file
        for _, apk, jar, apk_changed, jar_changed, _ in batch
